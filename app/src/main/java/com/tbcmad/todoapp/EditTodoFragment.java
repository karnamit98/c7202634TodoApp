...
public class    EditTodoFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rbLow = rootView.findViewById(R.id.edit_fragment_rb_low);
        rbMedium = rootView.findViewById(R.id.edit_fragment_rb_medium);
        rbHigh = rootView.findViewById(R.id.edit_fragment_rb_high);
    ...
    void SaveTodo(){
        ...
        boolean emptyTitle = txtTitle.getText().toString().trim().equals("");
        boolean emptyDescription = txtDescription.getText().toString().trim().equals("");
        int noPrioritySelected = checkedPriority;

        if(emptyTitle || emptyDescription || (noPrioritySelected==-1))
        {
            if(emptyTitle) txtTitle.setError("Enter the title!");
            if(emptyDescription) txtDescription.setError("Enter the description!");
            if(noPrioritySelected == -1) rbLow.setError("Select a priority!");

            return;
        }
        SharedPreferences preferences = getContext().getSharedPreferences("todo_pref", 0);
        int userId = preferences.getInt("userId", -1);
        eTodo.setUser_id(userId);
